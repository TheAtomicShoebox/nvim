...
function M.info()
  ...
  lines[#lines + 1] = "```"
  vim.notify(lines, vim.tbl_extend("force", { title = "Roots" }, { level = vim.log.levels.INFO }))
...
function M.setup()
  ...
  -- proabably because the event is triggered in the neo-tree buffer, therefore add `BufEnter`
