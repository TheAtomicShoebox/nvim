...
-- Git keymaps
-- Snacks is set up in bundles/ui; hunk-level
...
vim.keymap.set(
  "n",
  "<leader>gl",
  function() Snacks.picker.git_log({ cwd = root.git() }) end,
  { desc = "Git Log (root)" }
)
vim.keymap.set("n", "<leader>gL", function() Snacks.picker.git_log() end, { desc = "Git Log (cwd)" })

if vim.fn.executable("lazygit") == 1 then
  vim.keymap.set("n", "<leader>gg", function() Snacks.lazygit({ cwd = root.git() }) end, { desc = "Lazygit (root)" })
